...
import (
	...
	config "github.com/wso2/k8s-api-operator/api-operator/pkg/envoy/pkg/configs/confTypes"
	"io/ioutil"
	"os"
	"path/filepath"
	"sync"
...
func ReadLogConfigs() (*config.LogConfig, error) {
	once_lc.Do(func() {
		...
		mgwHome, _ := os.Getwd()
		//logger.Info("MGW_HOME: ", mgwHome)
		confF := filepath.Join(mgwHome, "/resources/conf/log_config.toml")
		_, err := os.Stat(confF)
		if err != nil {
			logger.Fatal("Log configuration file not found.", err)
		}
		content, readErr := ioutil.ReadFile(confF)
		if readErr != nil {
